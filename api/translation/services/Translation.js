...
module.exports = {
  ...
  add: async (values) => {
    const query = await Translation.create(_.omit(values, _.keys(_.groupBy(strapi.models.translation.associations, 'alias'))));
    const data = query.toJSON ? query.toJSON() : query;

    await strapi.hook.mongoose.manageRelations('translation', _.merge(data, { values }));
  ...
  edit: async (params, values) => {
    ...
    await strapi.hook.mongoose.manageRelations('translation', _.merge(_.clone(params), { values }));
    return Translation.update(params, values, { multi: true });
  },
  ...
  remove: async params => {
    ...
    const data = await Translation.findOneAndRemove(params, {})
      .populate(_.keys(_.groupBy(_.reject(strapi.models.translation.associations, {autoPopulate: false}), 'alias')).join(' '));

    _.forEach(Translation.associations, async association => {
