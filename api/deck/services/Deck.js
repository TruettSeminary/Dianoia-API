...
module.exports = {
  ...
  add: async (values) => {
    const query = await Deck.create(_.omit(values, _.keys(_.groupBy(strapi.models.deck.associations, 'alias'))));
    const data = query.toJSON ? query.toJSON() : query;

    await strapi.hook.mongoose.manageRelations('deck', _.merge(data, { values }));
  ...
  edit: async (params, values) => {
    ...
    await strapi.hook.mongoose.manageRelations('deck', _.merge(_.clone(params), { values }));
    return Deck.update(params, values, { multi: true });
  },
  ...
  remove: async params => {
    ...
    const data = await Deck.findOneAndRemove(params, {})
      .populate(_.keys(_.groupBy(_.reject(strapi.models.deck.associations, {autoPopulate: false}), 'alias')).join(' '));

    _.forEach(Deck.associations, async association => {
