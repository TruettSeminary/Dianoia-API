...
module.exports = {
  ...
  add: async (values) => {
    const query = await Card.create(_.omit(values, _.keys(_.groupBy(strapi.models.card.associations, 'alias'))));
    const data = query.toJSON ? query.toJSON() : query;

    await strapi.hook.mongoose.manageRelations('card', _.merge(data, { values }));
