...
const stripData = (userData) => {
  let data = userData; 
  if(data.classes) {
    data.classes = data.classes.map((clazz) => {
      return clazz._id; 
    }); 
  }
...
module.exports = {
  ...
  me: async (ctx) => {
    ...
    // TODO: monitor to ensure this does not mess up viewing users in admin panel (or elsewhere)
    // TODO: find better way to prevent population
    // Prevent population of nested elements from showing up on front end
